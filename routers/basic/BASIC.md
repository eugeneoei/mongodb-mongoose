# Basic `mongodb-mongoose` CRUD Endpoints

In this basic tutorial, the focus is on understanding the different approaches and implementations to query for data from `mongodb` through `mongoose`.

In `tweetsRouter.js` of this folder, I have structured routes for `GET`, `POST`, `PUT`, `PATCH` and `DELETE`. In summary:
...
In each of these routes, there are different implmentations written to achieve the same result. Uncomment them accordingly and experiment with the different implementations on your own.
...
In `mongoose`, you can query for results by chaining methods or by passing `options` into the methods. To be honest, I can't say which is more appropriate.
...
- Seed database with a `GET` request to `localhost:3000/seeds` endpoint or you can start with a empty database and make `POST` requests accordingly to `/tweets` endpoint
- Check that the database has been seeded with a `GET` request to `localhost:3000/tweets`. You should get an array of tweets.

# `GET /tweets` route

The following are the requirements I have set in this route to understand the different methods:
